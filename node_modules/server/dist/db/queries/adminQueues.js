...
export async function sumPendingBorrowUsdForUser(userId) {
    try {
        const { rows } = await pool.query(`SELECT COALESCE(SUM(amount_usd), 0)::text AS t
       FROM borrow_requests
       WHERE user_id = $1::uuid AND status = 'pending_admin'`, [userId]);
...
export async function insertBorrowRequest(input) {
    const { rows } = await pool.query(`INSERT INTO borrow_requests (
...
export async function listBorrowRequestsForUser(userId, limit = 50) {
    try {
        const { rows } = await pool.query(`SELECT id, asset, amount_usd::text, rate_mode, variable_apr::text, stable_apr::text, status, created_at
       FROM borrow_requests
...
export async function listPendingBorrowRequestsAdmin() {
    try {
        const { rows } = await pool.query(`SELECT br.id, br.user_id, u.email AS user_email, br.asset, br.amount_usd::text, br.rate_mode,
...
export async function approveBorrowRequestAdmin(input) {
    ...
    try {
        ...
        const r = await client.query(`SELECT id, user_id, asset, amount_usd::text, rate_mode, variable_apr::text, stable_apr::text, status
       FROM borrow_requests
       WHERE id = $1::uuid
       FOR UPDATE`, [input.requestId]);
        ...
        await client.query(`UPDATE borrow_requests
       SET status = 'approved', reviewed_at = NOW(), reviewed_by = $2::uuid
       WHERE id = $1::uuid`, [row.id, input.adminUserId]);
...
export async function rejectBorrowRequestAdmin(input) {
    const res = await pool.query(`UPDATE borrow_requests
     SET status = 'rejected', reviewed_at = NOW(), reviewed_by = $2::uuid, admin_notes = $3::text
     WHERE id = $1::uuid AND status = 'pending_admin'`, [input.requestId, input.adminUserId, input.reason.trim()]);
    return (res.rowCount ?? 0) > 0;
}
export async function insertTransferRequest(input) {
    const { rows } = await pool.query(`INSERT INTO transfer_requests (from_user_id, to_user_id, asset, amount, status)
     VALUES ($1::uuid, $2::uuid, $3::text, $4::numeric, 'pending_admin')
     RETURNING id`, [input.fromUserId, input.toUserId, input.asset, input.amount]);
    return { id: rows[0].id };
}
export async function listTransferRequestsForUser(userId, limit = 40) {
    try {
        const { rows } = await pool.query(`SELECT id, from_user_id, to_user_id, asset, amount::text, status, created_at
...
export async function listPendingTransferRequestsAdmin() {
    try {
        const { rows } = await pool.query(`SELECT tr.id, tr.from_user_id, tr.to_user_id, tr.asset, tr.amount::text, tr.status, tr.created_at,
...
export async function approveTransferRequestAdmin(input) {
    ...
    try {
        await client.query("BEGIN");
        const r = await client.query(`SELECT id, from_user_id, to_user_id, asset, amount::text, status
       FROM transfer_requests
       WHERE id = $1::uuid
       FOR UPDATE`, [input.requestId]);
        const row = r.rows[0];
        ...
        await client.query(`UPDATE transfer_requests
       SET status = 'approved', reviewed_at = NOW(), reviewed_by = $2::uuid
       WHERE id = $1::uuid`, [row.id, input.adminUserId]);
        await client.query("COMMIT");
        return { ok: true };
...
export async function rejectTransferRequestAdmin(input) {
    const res = await pool.query(`UPDATE transfer_requests
     SET status = 'rejected', reviewed_at = NOW(), reviewed_by = $2::uuid, admin_notes = $3::text
     WHERE id = $1::uuid AND status = 'pending_admin'`, [input.requestId, input.adminUserId, input.reason.trim()]);
    return (res.rowCount ?? 0) > 0;
}
export async function adminAdjustWalletBalance(input) {
    ...
    try {
       ...
       ON CONFLICT (user_id, currency) DO NOTHING`, [input.userId, asset]);
        const locked = await client.query(`SELECT balance::text AS b
       FROM wallets
       WHERE user_id = $1::uuid AND currency = $2::text
       FOR UPDATE`, [input.userId, asset]);
        if (locked.rowCount !== 1) {
            await client.query("ROLLBACK");
            return { ok: false, error: "Wallet row missing after insert" };
        }
        const oldBal = locked.rows[0].b;
        const appliedRow = await client.query(`SELECT (
         CASE
           WHEN ($1::numeric + $2::numeric) < 0 THEN (-($1::numeric))::text
           ELSE ($2::numeric)::text
         END
       ) AS applied,
       (($1::numeric + $2::numeric) < 0) AS was_clamped`, [oldBal, delta]);
        ...
        const upd = await client.query(`UPDATE wallets
       SET balance = balance + $3::numeric
       WHERE user_id = $1::uuid AND currency = $2::text
       RETURNING balance::text AS b`, [input.userId, asset, appliedStr]);
        if (upd.rowCount !== 1) {
            await client.query("ROLLBACK");
       ...
       VALUES ($1::uuid, $2::text, $3::ledger_direction, $4::numeric, 'admin_balance_adjust', 'admin', NULL)`, [input.userId, asset, dir, ledgerAmt]);
