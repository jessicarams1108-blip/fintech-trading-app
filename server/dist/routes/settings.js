...
import { findUserById, formatUserFullName, isUsernameAvailableForUser, parseFullName, updateUserNames, updateUsername, } from "../db/queries/users.js";
...
settingsRouter.patch("/profile", limiter, async (req, res, next) => {
    ...
    try {
        ...
        if (nameChanged)
            void sendAccountChangeEmail(u.email, "profile");
        if (usernameChanged)
            void sendAccountChangeEmail(u.email, "username");
        res.json({
            data: {
                ok: true,
