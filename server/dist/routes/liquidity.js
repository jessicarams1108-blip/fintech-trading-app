...
import { canBorrowNow, effectiveMaxBorrowUsd, availableBorrowUsd, MIN_SUPPLIED_USD_TO_BORROW, tierBorrowCap, } from "../lib/borrowPolicy.js";
...
liquidityRouter.get("/summary", limiter, authenticateRequired, async (req, res) => {
    try {
        ...
        const grossMaxBorrowUsd = Math.round(effectiveMaxBorrowUsd({
            suppliedUsd,
            kycStatus: kyc.kyc_status,
            kycTier: kyc.kyc_tier,
        }) * 100) / 100;
        const maxBorrowUsd = Math.round(availableBorrowUsd({
            suppliedUsd,
            kycStatus: kyc.kyc_status,
            kycTier: kyc.kyc_tier,
        ...
        const netSupplyApyPct = suppliedUsd > 0 ? Math.round((3.2 + Math.min(1.5, outstandingBorrowUsd / Math.max(suppliedUsd, 1)) * 0.8) * 10) / 10 : 0;
        res.json({
            suppliedUsd,
            outstandingBorrowUsd,
            grossMaxBorrowUsd,
            ...
            oTokensNote: "Inspired by Aave: supplied assets earn yield; Oove-oToken-style receipt tokens are on the roadmap.",
        });
    }
    catch (e) {
