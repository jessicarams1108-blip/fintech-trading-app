...
export const BORROW_CAP_BY_TIER = {
    0: 0,
    1: 30_000,
    2: 65_000,
    3: 100_000,
...
export function tierBorrowCap(tier) {
...
export function canBorrowNow(params) {
    ...
    if (params.suppliedUsd < MIN_SUPPLIED_USD_TO_BORROW) {
        reasons.push(`Supply at least $${MIN_SUPPLIED_USD_TO_BORROW.toLocaleString("en-US")} US equivalent in supported assets before borrowing.`);
    }
    const gross = effectiveMaxBorrowUsd({
        suppliedUsd: params.suppliedUsd,
        kycStatus: params.kycStatus,
