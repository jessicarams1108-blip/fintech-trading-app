...
import { getUserKyc, setKycPending } from "../db/queries/liquidity.js";
...
const idDocTypes = ["passport", "drivers_license", "national_id"] as const;
...
identityRouter.get("/status", limiter, async (req, res, next) => {
  try {
    ...
    let submission: Awaited<ReturnType<typeof getLatestSubmissionForUser>> = null;
    try {
...
identityRouter.post("/submit", limiter, async (req, res, next) => {
  ...
  try {
    ...
    if (parsed.data.dateOfBirth) vendorFields.dateOfBirth = parsed.data.dateOfBirth;
    if (parsed.data.nationality) vendorFields.nationality = parsed.data.nationality;
    if (parsed.data.occupation) vendorFields.occupation = parsed.data.occupation;
    if (parsed.data.sourceOfFunds) vendorFields.sourceOfFunds = parsed.data.sourceOfFunds;

    let idStorageKey: string;
    ...
    try {
      await pool.query(
        `INSERT INTO verification_documents (user_id, doc_type, storage_key, status)
         VALUES ($1::uuid, $2::text, $3::text, 'pending_review')`,
        [userId, parsed.data.idDocType, idStorageKey],
      );
    } catch {
      /* optional table */
    }
