...
import {
  ...
  findUserById,
  findPendingVerificationByEmail,
  isReservedUsername,
  isUsernameAvailable,
  markUserVerified,
  setUserOtp,
...
authRouter.post("/resend-otp", authLimiter, async (req, res) => {
  ...
  const body: { message: string; devVerificationCode?: string } = {
    message: "Verification code sent",
  };
  if (!env.RESEND_API_KEY) {
    body.devVerificationCode = code;
...
authRouter.post("/session", authLimiter, async (req, res) => {
  if (!env.DEV_PASSWORDLESS_SESSION) {
    res.status(400).json({
      error:
        "Passwordless sign-in is disabled. Sign in with your email and password, or set DEV_PASSWORDLESS_SESSION=true for legacy demos.",
    });
    return;
  }
