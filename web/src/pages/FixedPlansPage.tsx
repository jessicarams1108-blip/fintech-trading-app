...
import { fetchFixedPlans, fetchMyFixedPlans, type FixedPlan, type FixedSubscription } from "@/lib/fixedSavingsApi";
  ...
  return (
    <li className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white px-4 py-4 shadow-sm">
      ...
      <div className="min-w-0 flex-1">
        <p className="font-semibold text-slate-900">{rate}% Interest p.a.</p>
        <p className="text-sm text-slate-500">
          {plan.min_days} - {plan.max_days} Days Duration
        </p>
      </div>
      <Link
...
function MyPlanRow({ sub }: { sub: FixedSubscription }) {
  return (
    <li className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
      ...
      <p className="mt-2 text-lg font-semibold tabular-nums text-slate-900">{formatUsd(Number.parseFloat(sub.amount))}</p>
        ...
        <>
          <div>
            <h1 className="text-xl font-bold leading-snug text-slate-900">
              Fixed savings for a stable and rewarding future.
            </h1>
            <p className="mt-2 inline-flex items-center gap-1 text-sm font-medium text-emerald-600">
              <span aria-hidden>✓</span> Security Guaranteed
            </p>
          </div>
          ...
          <section className="rounded-2xl border border-violet-100 bg-violet-50/50 p-5">
            <h2 className="font-semibold text-slate-900">What is Fixed Savings?</h2>
            <p className="mt-2 text-sm text-slate-600">
              Securely save your funds for your preferred duration and enjoy high interest rates. The longer you save,
              the more you earn!
            </p>
